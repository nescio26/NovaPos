...
const RecentOrders = () => {
  const handleStatusChange = (index, value) => {
    console.log(`Updating order ${index} to ${value}`);
  };
  ...
  return (
    <div className="w-full bg-white dark:bg-[#16191D] p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm transition-all duration-300">
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <button className="group p-4 bg-[#F8F9FD] dark:bg-white/5 rounded-2xl text-[#FF5C00] hover:bg-[#FF5C00] hover:text-white transition-all active:scale-90 shadow-sm">
          <GrUpdate
            size={20}
        ...
        {orders.map((order, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-5 flex-1">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                  ...
                  {order.dateTime || "Just Now"}
                </div>
              </div>
            </div>

            {/* CENTER: STATUS & TABLE (PRIMARY FOCUS) */}
            <div className="flex flex-wrap items-center gap-3 my-4 lg:my-0 lg:flex-1 lg:justify-center">
              ...
              {/* STATUS SELECT */}
              <select
                className={`ml-2 text-[10px] font-black uppercase tracking-[0.1em] py-2.5 px-4 rounded-xl border-none focus:outline-none cursor-pointer shadow-sm transition-colors ${
                  order.status === "Ready"
            ...
            <div className="flex items-center justify-between lg:justify-end lg:flex-1 gap-6 pt-4 lg:pt-0 border-t lg:border-none border-slate-100 dark:border-white/5">
              ...
              <button className="h-12 w-12 bg-white dark:bg-[#0B0E11] rounded-2xl flex items-center justify-center text-slate-400 hover:text-[#FF5C00] border border-slate-100 dark:border-white/10 transition-all active:scale-90">
                <GrUpdate size={14} />
              </button>
            </div>
          </div>
